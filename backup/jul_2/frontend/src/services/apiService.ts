...
const getSettings = async (): Promise<Settings> => {
    try {
        ...
        const response = await apiClient.get<Settings>('/public/settings');
        return response.data;
    } catch (error: any) {
        console.error('Error fetching settings:', error);
        // Return default settings if the request fails
...
// Find user by email (for adding as author)
const findUserByEmail = async (email: string): Promise<UserRead> => {
    const response = await apiClient.get<UserRead>(`/entries/users/search-by-email`, {
        params: { email }
    });
    return response.data;
};
...
export const getOwnerUsers = async (): Promise<UserRead[]> => {
    const response = await apiClient.get<UserRead[]>('/admin/users/role/owner');
    return response.data;
};
...
export const addEntryAuthor = async (entryId: number, authorId: number): Promise<JournalEntryAuthorLinkRead> => {
    // Try the new entries endpoint first (supports all required roles)
    try {
        const response = await apiClient.post<JournalEntryAuthorLinkRead>(
            `/entries/${entryId}/authors`, 
            { user_id: authorId }
        );
        return response.data;
    } catch (error) {
        ...
        try {
            ...
            // Fallback: Try the editor endpoint
            const response = await apiClient.post<JournalEntryAuthorLinkRead>(
...
export const removeEntryAuthor = async (entryId: number, authorId: number): Promise<void> => {
    try {
        // Try the new entries endpoint first (supports all required roles)
        await apiClient.delete(`/entries/${entryId}/authors/${authorId}`);
    } catch (error) {
        // Fallback: Try the admin endpoint
        try {
