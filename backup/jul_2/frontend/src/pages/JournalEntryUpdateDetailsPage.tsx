...
import { formatDate, getRoleTranslation, getStatusTranslation } from '../utils/dateUtils';
...
const JournalEntryUpdateDetailsPage: React.FC = () => {
  ...
  const [authorUsersMap, setAuthorUsersMap] = useState<Map<number, apiService.UserRead>>(new Map());
  const [refereeUsersMap, setRefereeUsersMap] = useState<Map<number, apiService.UserRead>>(new Map());
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Redirect unauthenticated users to login since this page requires authentication
    ...
    fetchData();
  }, [entryId, user, isAuthenticated, navigate]);
  ...
  // Fetch missing author and referee user data
  useEffect(() => {
    const fetchMissingUserData = async () => {
      ...
      // Fetch missing author user data
      const newAuthorNamesMap = new Map(authorNamesMap);
      const newAuthorUsersMap = new Map(authorUsersMap);
      await Promise.all(
        Array.from(missingAuthorIds).map(async (authorId) => {
          try {
            const authorInfo = await apiService.getPublicUserInfo(authorId.toString());
            newAuthorNamesMap.set(authorId, authorInfo.name);
            newAuthorUsersMap.set(authorId, authorInfo);
          } catch (error) {
      ...
      setAuthorUsersMap(newAuthorUsersMap);

      // Fetch missing referee user data
      const newRefereeNamesMap = new Map(refereeNamesMap);
      const newRefereeUsersMap = new Map(refereeUsersMap);
      await Promise.all(
        Array.from(missingRefereeIds).map(async (refereeId) => {
          try {
            const refereeInfo = await apiService.getPublicUserInfo(refereeId.toString());
            newRefereeNamesMap.set(refereeId, refereeInfo.name);
            newRefereeUsersMap.set(refereeId, refereeInfo);
          } catch (error) {
      ...
      setRefereeUsersMap(newRefereeUsersMap);
    };
  ...
  useEffect(() => {
    ...
    const authorUpdatesCombined = authorUpdates.map(update => {
      ...
      const authorFromMap = authorUsersMap.get(update.author_id);
      const authorName = getAnonymizedUpdateAuthorName(update.author_id);
      const userRole = updateAuthor?.role || authorFromMap?.role || 'author';
    ...
    const refereeUpdatesCombined = refereeUpdates.map(update => {
      ...
      const refereeFromMap = refereeUsersMap.get(update.referee_id);
      const refereeName = getAnonymizedUpdateRefereeName(update.referee_id);
      const userRole = updateReferee?.role || refereeFromMap?.role || 'referee';
      const isWithinWindow = isWithinDeletionWindow(update.created_date);
  ...
  }, [authorUpdates, refereeUpdates, authors, referees, user, authorNamesMap, refereeNamesMap, authorUsersMap, refereeUsersMap]);
  ...
  const handleCopyUpdate = async (update: CombinedUpdate) => {
    ...
    copyText += `By: ${update.type === 'author' ? 
      (update.authorId ? getAnonymizedUpdateAuthorName(update.authorId) : update.authorName) : 
      (update.refereeId ? getAnonymizedUpdateRefereeName(update.refereeId) : update.refereeName)}\n\n`;
  ...
  // Helper function to get anonymized author name
  const getAnonymizedAuthorName = (author: apiService.UserRead, index: number): string => {
    const currentUserIsReferee = user && referees.some(referee => referee.id === user.id);
    if (currentUserIsReferee) {
      return language === 'tr' ? `Yazar-${index + 1}` : `Author-${index + 1}`;
    }
    return author.name;
  };

  // Helper function to get anonymized referee name
  const getAnonymizedRefereeName = (referee: apiService.UserRead, index: number): string => {
    const currentUserIsAuthor = user && authors.some(author => author.id === user.id);
    const currentUserIsReferee = user && referees.some(ref => ref.id === user.id);
    
    if (currentUserIsAuthor || currentUserIsReferee) {
      return language === 'tr' ? `Hakem-${index + 1}` : `Referee-${index + 1}`;
    }
    return referee.name;
  };

  // Helper function to get anonymized name for updates
  const getAnonymizedUpdateAuthorName = (authorId: number): string => {
    const currentUserIsReferee = user && referees.some(referee => referee.id === user.id);
    if (currentUserIsReferee) {
      // Find the index of this author in the authors array
      const authorIndex = authors.findIndex(author => author.id === authorId);
      if (authorIndex !== -1) {
        return language === 'tr' ? `Yazar-${authorIndex + 1}` : `Author-${authorIndex + 1}`;
      }
      // If not found in current authors, check the maps
      const mapKeys = Array.from(authorUsersMap.keys()).sort();
      const mapIndex = mapKeys.findIndex(key => key === authorId);
      if (mapIndex !== -1) {
        return language === 'tr' ? `Yazar-${mapIndex + 1}` : `Author-${mapIndex + 1}`;
      }
      return language === 'tr' ? 'Yazar' : 'Author';
    }
    
    // Return actual name for non-referees
    const author = authors.find(a => a.id === authorId);
    if (author) return author.name;
    return authorNamesMap.get(authorId) || 'Unknown Author';
  };

  // Helper function to get anonymized name for referee updates
  const getAnonymizedUpdateRefereeName = (refereeId: number): string => {
    const currentUserIsAuthor = user && authors.some(author => author.id === user.id);
    const currentUserIsReferee = user && referees.some(referee => referee.id === user.id);
    
    if (currentUserIsAuthor || currentUserIsReferee) {
      // Find the index of this referee in the referees array
      const refereeIndex = referees.findIndex(referee => referee.id === refereeId);
      if (refereeIndex !== -1) {
        return language === 'tr' ? `Hakem-${refereeIndex + 1}` : `Referee-${refereeIndex + 1}`;
      }
      // If not found in current referees, check the maps
      const mapKeys = Array.from(refereeUsersMap.keys()).sort();
      const mapIndex = mapKeys.findIndex(key => key === refereeId);
      if (mapIndex !== -1) {
        return language === 'tr' ? `Hakem-${mapIndex + 1}` : `Referee-${mapIndex + 1}`;
      }
      return language === 'tr' ? 'Hakem' : 'Referee';
    }
    
    // Return actual name for admins/editors
    const referee = referees.find(r => r.id === refereeId);
    if (referee) return referee.name;
    return refereeNamesMap.get(refereeId) || 'Unknown Referee';
  };
          ...
          <div className="chat-header-info">
            ...
            <div className="chat-subtitle">
              ...
              <div 
                ...
                onMouseEnter={() => {
                  // Only show tooltip if user is not an author or referee
                  const currentUserIsAuthor = user && authors.some(author => author.id === user.id);
                  const currentUserIsReferee = user && referees.some(referee => referee.id === user.id);
                  if (!currentUserIsAuthor && !currentUserIsReferee) {
                          ...
                          {authors.map((author) => (
                            <div key={`author-${author.id}`} style={{
                              ...
                              <div style={{
                                ...
                                {getUserInitials(getAnonymizedAuthorName(author, authors.indexOf(author)))}
                              </div>
                              <span style={{
                                fontSize: '13px',
                                fontWeight: '500',
                              ...
                              }}>{getAnonymizedAuthorName(author, authors.indexOf(author))}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                          ...
                          {referees.map((referee) => (
                            <div key={`referee-${referee.id}`} style={{
                              ...
                              <div style={{
                                ...
                                {getUserInitials(getAnonymizedRefereeName(referee, referees.indexOf(referee)))}
                              </div>
                              <span style={{
                                fontSize: '13px',
                                fontWeight: '500',
                                ...
                                display: 'block',
                                minWidth: 'max-content'
                              }}>{getAnonymizedRefereeName(referee, referees.indexOf(referee))}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                >
                  <div className="message-avatar">
                    <div className={`avatar-circle ${update.type}`}>
                      {update.type === 'author' 
                        ? getUserInitials(update.authorId ? getAnonymizedUpdateAuthorName(update.authorId) : update.authorName || '') 
                        : getUserInitials(update.refereeId ? getAnonymizedUpdateRefereeName(update.refereeId) : update.refereeName || '')
                      }
                    </div>
                  </div>
                      ...
                      {update.type === 'author' ? (
                        ...
                        <>
                          <div className="message-sender">
                            <span className="sender-name">
                              {getAnonymizedUpdateAuthorName(update.authorId || 0)}
                            </span>
                            <span className={`sender-role ${update.userRole || update.type}`}>
                              {getRoleTranslation(update.userRole || 'author', language)}
                            </span>
                          </div>
                        ...
                        <>
                          ...
                          <div className="message-sender referee-sender">
                            <span className={`sender-role ${update.userRole || update.type}`}>
                              {getRoleTranslation(update.userRole || 'referee', language)}
                            </span>
                            <span className="sender-name">
                              {getAnonymizedUpdateRefereeName(update.refereeId || 0)}
                            </span>
                          </div>
                        </>
                      )}
                          ...
                          {update.file_path && update.canViewFile ? (
                            <div className={`detail-item ${update.type === 'referee' ? 'detail-item-file-referee' : 'detail-item-file-author'}`}>
                              ...
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                          if (update.file_path?.toLowerCase().endsWith('.docx')) {
          downloadFile(update.file_path, 'güncelleme-dosyası.docx');
                                  } else {
                                    viewFile(update.file_path!);
                                  }
                                }}
                                className="file-link"
                                style={{
