...
const JournalEditFormPage: React.FC = () => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            // Get the current journal data to check if publication date is being changed
            const currentJournal = await apiService.getJournals().then(journals => 
            ...
            const oldPublicationDate = currentJournal?.publication_date;
            const newPublicationDate = cleanedFormData.publication_date;
            const publicationDateChanged = oldPublicationDate !== newPublicationDate;
            
            // First update the journal data
            setUploadProgress(language === 'tr' ? 'Dergi bilgileri güncelleniyor...' : 'Updating journal data...');
            await apiService.updateJournal(journalId, cleanedFormData);

            // If publication date changed, update all accepted entries in this journal
            if (publicationDateChanged && newPublicationDate) {
                try {
                    setUploadProgress(language === 'tr' ? 'Makale yayın tarihleri güncelleniyor...' : 'Updating entry publication dates...');
                    // Get all entries for this journal
                    const journalEntries = await apiService.getEntriesByJournal(journalId);
                    
                    // Filter for accepted entries only
                    const acceptedEntries = journalEntries.filter(entry => entry.status === 'accepted');
                    
                    // Update publication date for each accepted entry
                    const updatePromises = acceptedEntries.map(entry =>
                        apiService.updateEntry(entry.id, {
                    ...
                    // Show success message for entries update
                    if (acceptedEntries.length > 0) {
                        setToastMessage(language === 'tr' 
                            ? `Dergi ve ${acceptedEntries.length} kabul edilmiş makalenin yayın tarihi güncellendi.`
                            : `Journal and ${acceptedEntries.length} accepted entries publication date updated.`);
                        setToastType('success');
                        setShowToast(true);
                        setTimeout(() => setShowToast(false), 5000);
                    }
                } catch (entriesError) {
                    console.error('Error updating entries publication dates:', entriesError);
                    setToastMessage(language === 'tr' 
                        ? 'Dergi güncellendi ancak makale yayın tarihleri güncellenirken hata oluştu.'
                        : 'Journal updated but error occurred while updating entries publication dates.');
                    setToastType('warning');
                    setShowToast(true);
                    setTimeout(() => setShowToast(false), 5000);
                }
            ...
            if (hasFiles) {
                ...
                setUploadProgress(language === 'tr' ? 'Dosyalar yükleniyor...' : 'Uploading files...');
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        ...
                        }}>{language === 'tr' ? 'Dergi Bulunamadı!' : 'Journal Not Found!'}</h1>
                        ...
                        <p style={{
                            ...
                            marginBottom: '32px',
                            fontWeight: '500'
                        }}>{language === 'tr' ? 'Bu dergi düzenlenemez veya mevcut değil.' : 'This journal cannot be edited or does not exist.'}</p>
                            ...
                            <div className="form-group" key={key}>
                                ...
                                {(formData as any)[key] && (
                                    <div style={{ 
                                        ...
                                        <button 
                                            onClick={() => viewFile((formData as any)[key])}
                                            style={{
                                                color: color,
                                                background: 'none',
                                                border: 'none',
                            ...
                            >
                                {isSubmitting ? (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        ...
                                        <span>{uploadProgress || (language === 'tr' ? 'Kaydediliyor...' : 'Saving...')}</span>
                                    </div>
