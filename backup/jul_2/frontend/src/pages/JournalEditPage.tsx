...
const JournalEditPage: React.FC = () => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            setUploadProgress(t('updatingEntryData') || 'Updating entry data...');
            await apiService.updateEntry(Number(id), cleanedFormData);

            // Then handle file uploads if any files were selected
            const hasFiles = selectedFiles.file_path || selectedFiles.full_pdf;
            if (hasFiles) {
                setIsUploadingFiles(true);
                
                if (selectedFiles.file_path) {
                    setUploadProgress(t('uploadingDocxFile') || 'Uploading DOCX file...');
                ...
                if (selectedFiles.full_pdf) {
                    setUploadProgress(t('uploadingPdfFile') || 'Uploading PDF file...');
                            ...
                            >
                                {isSubmitting ? (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        ...
                                        <span>{uploadProgress || (t('saving') || 'Saving...')}</span>
                                    </div>
                                ) : (t('updateEntry') || 'Update Entry')}
                            </button>
                        </div>
                    </form>
                </div>
