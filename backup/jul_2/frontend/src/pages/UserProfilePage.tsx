            ...
            try {
                ...
                if (!userId) {
                    ...
                    try {
                        ...
                        try {
                            // First try the dedicated API for user's edited journals
                            const journals = await apiService.getMyEditedJournals();
                            // Separate editor-in-chief journals from regular editor journals
                            ...
                            // Fallback to the previous complex approach for compatibility
                            try {
                                // For all users, check if they are editors of any journals
                                ...
                                // Final fallback to editor-specific API
                                try {
                    ...
                    try {
                        ...
                        if (targetUser) {
                            ...
                            // Always try to fetch editor journals for the target user
                            try {
                                ...
                                // Editor-in-chief journals: user is editor-in-chief
                                const userEditorInChiefJournals = allJournals.filter(journal => 
                                    journal.editor_in_chief_id === Number(userId)
                                );
        ...
        <>
            ...
            <div className="page-content-section" style={{ marginLeft: '60px' }}>
                ...
                <div className="user-info-card">
                    ...
                    <div className="profile-actions-corner">
                        ...
                        {/* Admin actions for other user profiles - with permission checks */}
                        {userId && profileUser && user && (user?.role === 'admin' || user?.role === 'owner') && 
                            // Don't show admin actions when viewing own profile
                            user.id !== Number(userId) && 
                            // Owners can edit anyone, Admins can edit everyone except owners
                            (user.role === 'owner' || (user.role === 'admin' && profileUser.role !== 'owner')) && (
                            <button 
            ...
            {editorInChiefJournals.length > 0 && (
                <div className="entries-section">
                    <h2>{userId ? (language === 'tr' ? 'BaşEditör Olunan Dergiler' : 'Editor-in-Chief Journals') : (language === 'tr' ? 'BaşEditör Olduğum Dergiler' : 'Editor-in-Chief Journals')}</h2>
