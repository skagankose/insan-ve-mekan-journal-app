...
const RecordsPage: React.FC = () => {
    ...
    const [showPublishedEditorInChiefJournals, setShowPublishedEditorInChiefJournals] = useState<boolean>(true);
    const [showDraftEditorInChiefJournals, setShowDraftEditorInChiefJournals] = useState<boolean>(true);
        ...
        const fetchAllData = async () => {
            ...
            try {
                ...
                try {
                    ...
                    // Always try to fetch both editor and editor-in-chief journals
                    try {
                        // Try the comprehensive approach first: get all journals and filter by user roles
                        ...
                        // Regular editor journals: user is in editor links but NOT editor-in-chief
                        const userRegularEditorJournals = allJournals.filter(journal => 
                            userJournalIds.includes(journal.id) && journal.editor_in_chief_id !== user.id
                        );
                        
                        // Editor-in-chief journals: user is editor-in-chief
                        const userEditorInChiefJournals = allJournals.filter(journal => 
                            journal.editor_in_chief_id === user.id
                        );
                        
                        setEditorJournals(userRegularEditorJournals);
                        setEditorInChiefJournals(userEditorInChiefJournals);
                    } catch (err) {
                        console.error("Failed to fetch comprehensive journal data:", err);
                        try {
                            // Fallback 1: Try getMyEditedJournals (may only return regular editor journals)
                            const [myEditedJournals, allJournals] = await Promise.all([
                                apiService.getMyEditedJournals(),
                                apiService.getAllJournals()
                            ]);
                            
                            // Get editor-in-chief journals from all journals
                            const editorInChiefJournals = allJournals.filter(journal => 
                                journal.editor_in_chief_id === user.id
                            );
                            
                            // Regular editor journals from getMyEditedJournals, excluding editor-in-chief roles
                            const regularEditorJournals = myEditedJournals.filter(journal => 
                                journal.editor_in_chief_id !== user.id
                            );
                            
                            setEditorJournals(regularEditorJournals);
                            setEditorInChiefJournals(editorInChiefJournals);
                        } catch (fallbackErr) {
                            console.error("Failed to fetch journals via fallback:", fallbackErr);
                            try {
                                // Fallback 2: Try getEditorJournals
                                const journals = await apiService.getEditorJournals();
                                const regularEditorJournals = journals.filter(journal => journal.editor_in_chief_id !== user.id);
                                const editorInChiefJournals = journals.filter(journal => journal.editor_in_chief_id === user.id);
                                setEditorJournals(regularEditorJournals);
                                setEditorInChiefJournals(editorInChiefJournals);
                            } catch (editorErr) {
                                console.error("Failed to fetch editor journals via editor API:", editorErr);
                                setEditorJournals([]);
                                setEditorInChiefJournals([]);
                            }
                        }
                    }
                } catch (err: any) {
    ...
    return (
        <>
            ...
            <div className="page-content-section" style={{ marginLeft: '60px' }}>
                ...
                {editorJournals.length === 0 && editorInChiefJournals.length === 0 && userEntries.length === 0 && refereeEntries.length === 0 ? (
                    ...
                    <>
                        {/* Show editor-in-chief journals section only if user has editor-in-chief journals */}
                        {editorInChiefJournals.length > 0 && (
                            <div className="entries-section">
                                <h2>{language === 'tr' ? 'BaşEditör Olduğum Dergiler' : 'Editor-in-Chief Journals'}</h2>
                                {renderGroupedJournals(
                                    editorInChiefJournals,
                                    language === 'tr' ? 'Dergi Bulunamadı' : 'No Journals Found',
                                    showPublishedEditorInChiefJournals,
                                    setShowPublishedEditorInChiefJournals,
                                    showDraftEditorInChiefJournals,
                                    setShowDraftEditorInChiefJournals
                                )}
                            </div>
                        )}
