...
const JournalDetailsPage: React.FC = () => {
    ...
    const { activeJournal, refreshActiveJournal } = useActiveJournal();
    ...
    const [adminAndOwnerUsers, setAdminAndOwnerUsers] = useState<apiService.UserRead[]>([]);
    ...
    useEffect(() => {
        const fetchJournalAndEntries = async () => {
            ...
            try {
                ...
                // Try to fetch from published journals first
                try {
                    ...
                    console.error("Failed to fetch published journal data:", err);
                }
                
                // If not found in published journals, try to fetch from all journals
                if (!journalData) {
                    // Try different strategies based on authentication status
                    if (isAuthenticated) {
                        try {
                            const [allJournals, allEntries] = await Promise.all([
                                apiService.getJournals(),
                                apiService.getEntriesByJournal(parseInt(journalId))
                            ]);
                            journalData = allJournals.find(j => j.id === parseInt(journalId));
                            if (journalData) {
                                entriesData = allEntries;
                            }
                        } catch (err) {
                            console.error("Failed to fetch journal from all journals:", err);
                            
                            // If that fails too, try editor journals for authenticated editors
                            if (isEditor) {
                                try {
                                    const editorJournals = await apiService.getEditorJournals();
                                    journalData = editorJournals.find(j => j.id === parseInt(journalId));
                                    if (journalData) {
                                        const fetchedEntries = await apiService.getEntriesByJournal(parseInt(journalId));
                                        entriesData = fetchedEntries;
                                    }
                                } catch (editorErr) {
                                    console.error("Editor failed to fetch assigned journals:", editorErr);
                                }
                            }
                        }
                    } else {
                        // For non-authenticated users, if journal not found in published journals,
                        // it might be an unpublished journal - show access denied instead of not found
                        console.log("Non-authenticated user trying to access journal not in published list");
                        // Set a flag to show access denied for non-authenticated users
                        setAccessDenied(true);
                        setAccessDeniedReason(
                            language === 'tr' 
                                ? 'Bu yayınlanmamış dergiye erişim yetkiniz bulunmuyor. Uygulamaya giriş yapınız.' 
                                : 'You do not have permission to access this unpublished journal. Log in to the application.'
                        );
                        setLoading(false);
                        return;
                    }
                ...
                // If we found the journal, set it and fetch additional data
                if (journalData) {
                    setJournal(journalData);
                    setEntries(entriesData || []);
                    
                    // Fetch additional journal data
                    ...
                    // Journal not found anywhere
                    setError(t('journalNotFound') || 'Journal not found');
                }
    ...
    }, [journalId, isEditor, isAdmin, isAuthenticated, user, t]);

    // Access control effect - runs after journal and editors data are loaded
    useEffect(() => {
        if (!loading && journal) {
            // For published journals, allow access to everyone
            if (journal.is_published) {
                setAccessDenied(false);
                return;
            }
            
            // For unpublished journals, check user role (no authentication redirect)
            ...
            // Check user role for unpublished journals
            const restrictedRoles = ['author', 'referee'];
            
            // Admin and Owner have full access
            if (user.role === 'admin' || user.role === 'owner') {
                setAccessDenied(false);
            } else if (user.role === 'editor') {
                // For editors, check if they are assigned to this journal
                const isEditorInChief = journal.editor_in_chief_id === user.id;
                const isAssignedEditor = editors.some(editor => editor.id === user.id);
                
                if (isEditorInChief || isAssignedEditor) {
                    setAccessDenied(false);
                } else {
                    setAccessDenied(true);
                    setAccessDeniedReason(
                        language === 'tr' 
                            ? 'Bu dergiye editör olarak atanmamışsınız.' 
                            : 'You are not assigned as an editor to this journal.'
                    );
                }
                ...
                setAccessDeniedReason(
                    language === 'tr' 
                        ? 'Bu yayınlanmamış dergiye erişim yetkiniz bulunmuyor.' 
                        : 'You do not have permission to access this unpublished journal.'
                );
            } else {
                ...
                setAccessDeniedReason(
                    language === 'tr' 
                        ? 'Bu dergiye erişim yetkiniz bulunmuyor.' 
                        : 'You do not have permission to access this journal.'
                );
            }
        }
    }, [loading, journal, editors, isAuthenticated, user, language]);
        ...
        const fetchUsers = async () => {
            ...
            if (showEditorInChiefModal) {
                try {
                    ...
                    // Combine admin and owner users only (no editors), removing duplicates by ID
                    const combinedUsers = [...admins, ...owners].reduce((acc, user) => {
                        if (!acc.find(existingUser => existingUser.id === user.id)) {
                            acc.push(user);
                        }
                        return acc;
                    }, [] as apiService.UserRead[]);
                    setAdminAndOwnerUsers(combinedUsers);
                    if (journal?.editor_in_chief_id) {
                        setSelectedEditorInChiefId(journal.editor_in_chief_id);
                    }
                } catch (err) {
                    console.error('Failed to fetch admin and owner users:', err);
                }
            }
            
            if (showEditorsModal) {
                try {
                    const [editorRoleUsers, admins, owners] = await Promise.all([
                        apiService.getEditorRoleUsers(),
                        apiService.getAdminUsers(),
                        apiService.getOwnerUsers()
                    ]);
                    // Combine editor, admin and owner users, removing duplicates by ID
                    const combinedEditorUsers = [...editorRoleUsers, ...admins, ...owners].reduce((acc, user) => {
                        if (!acc.find(existingUser => existingUser.id === user.id)) {
                            acc.push(user);
                        }
                        return acc;
                    }, [] as apiService.UserRead[]);
                    setEditorUsers(combinedEditorUsers);
                    setSelectedEditorIds(editors.map(editor => editor.id));
                } catch (err) {
                    console.error('Failed to fetch editor, admin and owner users:', err);
                }
            }
        };
    ...
    const handleSetActive = async () => {
        ...
        try {
            // Update the backend settings first
            await apiService.updateSettings({ active_journal_id: journal.id });
            
            // Refresh the active journal from backend to ensure global state is updated
            await refreshActiveJournal();
                ...
                <div style={{
                    ...
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        <div style={{
                            ...
                            <svg width="60" height="60" viewBox="0 0 24 24" fill="none">
                                <path d="M16 10V6C16 5.46957 15.7893 4.96086 15.4142 4.58579C15.0391 4.21071 14.5304 4 14 4H10C9.46957 4 8.96086 4.21071 8.58579 4.58579C8.21071 4.96086 8 5.46957 8 6V10M5 10H19C20.1046 10 21 10.8954 21 12V19C21 20.1046 20.1046 21 19 21H5C3.89543 21 3 20.1046 3 19V12C3 10.8954 3.89543 10 5 10Z" 
                                    stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                        </div>
                        ...
                        <h1 style={{
                            ...
                            WebkitBackgroundClip: 'text',
                            WebkitTextFillColor: 'transparent'
                        }}>{language === 'tr' ? 'Erişim Reddedildi!' : 'Access Denied!'}</h1>
            ...
            <div className="page-title-section" style={{ marginLeft: '60px' }}>
                <div className="page-title-header">
                    ...
                    <div className="page-title-actions">
                        ...
                        {journal.full_pdf && (
                            <button 
                                onClick={() => downloadFile(journal.full_pdf!, 'dergi.pdf')}
                                style={{
                                    display: 'flex',
                                    alignItems: 'center',
                                    gap: '8px',
                                ...
                                {editorInChief.title && (
                                    ...
                                    <div style={{
                                        ...
                                        <div style={{
                                            ...
                                            justifyContent: 'center',
                                            fontSize: '20px'
                                        }}><HiAcademicCap size={24} color="#64748B" /></div>
                                        <p style={{
                                            margin: 0,
                                            fontSize: '15px',
                                            fontWeight: '500',
                ...
                <div style={{
                    ...
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        ...
                        <div style={{
                            ...
                            {editors.filter(editor => editor.id !== journal?.editor_in_chief_id).length > 0 ? (
                                editors.filter(editor => editor.id !== journal?.editor_in_chief_id).map(editor => (
                                    <div 
                                        key={editor.id} 
                ...
                {canManageJournal && (
                    <div style={{
                        ...
                        <div style={{
                            ...
                            {journal.cover_photo && (
                                <button 
                                    onClick={() => viewFile(journal.cover_photo!)}
                                    style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '12px',
                            ...
                            {journal.meta_files && (
                                <button 
                                    onClick={() => downloadFile(journal.meta_files!, 'meta-dosyalar.docx')}
                                    style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '12px',
                            ...
                            {canManageJournal && journal.editor_notes && (
                                <button 
                                    onClick={() => downloadFile(journal.editor_notes!, 'editör-notları.docx')}
                                    style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '12px',
                            ...
                            {journal.full_pdf && (
                                <button 
                                    onClick={() => downloadFile(journal.full_pdf!, 'dergi-tam.pdf')}
                                    style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '12px',
                            ...
                            {journal.index_section && (
                                <button 
                                    onClick={() => downloadFile(journal.index_section!, 'içindekiler.docx')}
                                    style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '12px',
                            ...
                            {journal.file_path && (
                                <button 
                                    onClick={() => downloadFile(journal.file_path!, 'birleştirilmiş-dosya.docx')}
                                    style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '12px',
                            ...
                            {adminAndOwnerUsers.length === 0 ? (
                                <div style={{
                                    ...
                                    <p style={{ margin: 0, fontSize: '16px', fontWeight: '500' }}>
                                        {t('noAdminOrOwnerUsers') || 'No admin or owner users found'}
                                    </p>
                                </div>
                            ) : (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                    {adminAndOwnerUsers.map(admin => (
                                        <label 
                                            key={admin.id} 
                        ...
                        <div style={{ 
                            ...
                            {editorUsers.length === 0 ? (
                                <div style={{
                                    ...
                                    <p style={{ margin: 0, fontSize: '16px', fontWeight: '500' }}>
                                        {language === 'tr' ? 'Editör, yönetici veya kurucu kullanıcısı bulunamadı' : 'No editor, admin or owner users found'}
                                    </p>
                                </div>
