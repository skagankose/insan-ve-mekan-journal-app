...
from .. import crud
from ..email_utils import SENDER_EMAIL, SENDER_NAME, send_login_link_email as send_login_link_email_util, send_journal_editor_assignment_notification, send_referee_assignment_to_referee_notification
...
def set_journal_editor_in_chief(
    ...
    if data.editor_in_chief_id:
        ...
        # Verify editor_in_chief has correct role (admin or owner only)
        if editor_in_chief.role not in [models.UserRole.admin, models.UserRole.owner]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Editor in chief must have admin or owner role"
            )
    ...
    # Send email notification to the new editor-in-chief
    if data.editor_in_chief_id and editor_in_chief:
        try:
            import os
            from ..email_utils import send_editor_in_chief_assignment_notification
            ...
            if api_key:
                ...
                send_editor_in_chief_assignment_notification(
                    api_key=api_key,
                ...
                print(f"Editor-in-Chief assignment notification sent to {editor_in_chief.email}")
            else:
                print("Warning: BREVO_API_KEY not found in environment variables. Email notification not sent.")
        except Exception as e:
            # Log the error but don't fail the main operation
            print(f"Failed to send editor-in-chief assignment notification to {editor_in_chief.email}: {e}")
...
def add_journal_editor(
    ...
    # Check if the user exists and has appropriate role (admin, owner, or editor)
    ...
    if editor.role not in [models.UserRole.admin, models.UserRole.owner, models.UserRole.editor]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="User must have admin, owner, or editor role to be added as an editor"
        )
    ...
    try:
        ...
        if api_key:
            ...
            send_journal_editor_assignment_notification(
                api_key=api_key,
            ...
            print(f"Journal editor assignment notification sent to {editor.email}")
        else:
            print("Warning: BREVO_API_KEY not found in environment variables. Email notification not sent.")
    except Exception as e:
        # Log the error but don't fail the main operation
        print(f"Failed to send journal editor assignment notification to {editor.email}: {e}")
