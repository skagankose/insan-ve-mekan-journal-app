...
def notify_on_entry_assigned_to_journal(
    db: Session,
    entry_id: int,
    journal_id: int
):
    """
    Notify editor-in-chief and editors when a new entry is assigned to their journal.
    
    Args:
        db: Database session
        entry_id: ID of the journal entry that was assigned
        journal_id: ID of the journal that the entry was assigned to
    """
    # Get the journal entry
    entry = db.query(models.JournalEntry).filter(models.JournalEntry.id == entry_id).first()
    if not entry:
    ...
    # Get author names for the entry
    authors = entry.authors
    ...
    if journal.editor_in_chief_id:
        ...
        if editor_in_chief:
            try:
                email_utils.send_entry_assigned_to_journal_notification(
                    api_key=BREVO_API_KEY,
                    user_email=editor_in_chief.email,
                    user_name=editor_in_chief.name,
                    entry_title=entry.title,
    ...
    # Notify editors (excluding editor-in-chief who was already notified)
    for editor in editors:
        if not journal.editor_in_chief_id or editor.id != journal.editor_in_chief_id:
            try:
                email_utils.send_entry_assigned_to_journal_notification(
                    api_key=BREVO_API_KEY,
                    user_email=editor.email,
