...
def create_entry(db: Session, entry: schemas.JournalEntryCreate, user_id: int) -> models.JournalEntry:
    ...
    # Send notification to journal editors if entry is assigned to a journal
    if db_entry.journal_id:
        try:
            from . import notification_utils
            notification_utils.notify_on_entry_assigned_to_journal(
                db=db,
            ...
            print(f"Failed to send journal assignment notification: {e}")
...
def update_entry(db: Session, entry_id: int, entry_update: schemas.JournalEntryUpdate) -> models.JournalEntry | None:
    ...
    # Store the old status and journal_id for comparison
    old_status = db_entry.status
    old_journal_id = db_entry.journal_id
    ...
    journal_id_changed = False
    new_journal_id = None
    if 'journal_id' in update_data:
        new_journal_id = update_data.pop('journal_id', None)
        if new_journal_id != old_journal_id:
            journal_id_changed = True
            db_entry.journal_id = new_journal_id
    ...
    # Send notification to journal editors if entry is newly assigned to a journal
    if journal_id_changed and new_journal_id is not None:
        try:
            from . import notification_utils
            notification_utils.notify_on_entry_assigned_to_journal(
                db=db,
