...
const JournalEntryDetailsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!canViewUserLists) return;
      ...
      if (showAuthorsModal) {
        try {
          // Use editor endpoint if user is editor, otherwise use admin endpoint
          const authors = isAdmin ? 
            await apiService.getAuthorUsers() : 
            (await apiService.getEditorUsers()).filter(user => user.role === 'author');
      ...
      if (showRefereesModal) {
        try {
          // Use editor endpoint if user is editor, otherwise use admin endpoint
          const referees = isAdmin ? 
            await apiService.getRefereeUsers() : 
            (await apiService.getEditorUsers()).filter(user => user.role === 'referee');
      ...
      if (showJournalModal) {
        try {
          // Use the appropriate function to fetch journals based on user role
                  const journalsData = hasJournalManagementAccess
  ...
  useEffect(() => {
    const isModalOpen = showAuthorsModal || showRefereesModal || showJournalModal || showAuthorDetailsModal || showRefereeDetailsModal || showCitationModal;
  ...
  }, [showAuthorsModal, showRefereesModal, showJournalModal, showAuthorDetailsModal, showRefereeDetailsModal, showCitationModal]);
